...
struct ContentView: View {
    ...
    var body: some View {
        GeometryReader { geometry in
            HStack(spacing: 0) {
                // 左侧控制面板
                if !isFullscreen {
                    ControlPanelView(model: teleprompterModel, showingTextInput: $showingTextInput)
                        .frame(width: 300)
                        .background(Color(NSColor.controlBackgroundColor))
                }
                
                // 右侧文本显示区域
                TextDisplayView(model: teleprompterModel, isFullscreen: $isFullscreen)
    ...
    private func setupInitialText() {
        ...
        这里有一些技术术语来测试识别准确性：Swift编程语言、macOS系统、NSApplication、语音识别框架、人工智能、机器学习。
